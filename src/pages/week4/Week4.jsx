...
const getProducts = (token, page = 1) =>
  axios.get(`${API_BASE}/api/${API_PATH}/admin/products?page=${page}`, { headers: { Authorization: token } });
...
function Week4() {
  ...
  // 頁碼相關
  const handleChangePage = async (e, targetPage) => {
    e.preventDefault();
    if (!targetPage || targetPage > pagination.total_pages) return;
    ...
    try {
      setIsLoading(true);
      setLoadingMessage(`正在前往第${targetPage}頁...`);

      const response = await getProducts(token, targetPage);
      setProducts(response.data.products);
      setPagination(response.data.pagination);
    } catch (error) {
      alert(`出現錯誤：${error.response?.data?.message || error.code || '未知錯誤'}`);
